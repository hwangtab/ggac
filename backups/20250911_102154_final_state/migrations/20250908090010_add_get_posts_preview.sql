-- Create a function to return lightweight post previews without full content payload
-- Safe to run multiple times
create or replace function public.get_posts_preview(p_category text default '전체', p_limit int default 20)
returns table (
  id uuid,
  title text,
  content_preview text,
  content_format text,
  category text,
  author_id uuid,
  created_at timestamptz,
  updated_at timestamptz,
  is_pinned boolean,
  like_count integer,
  author_display_name text
) language sql stable as $$
  with base as (
    select
      p.id,
      p.title,
      -- Strip HTML tags and truncate to ~150 chars for preview
      left(regexp_replace(coalesce(p.content, ''), '<[^>]*>', '', 'g'), 150) as content_preview,
      p.content_format,
      p.category,
      p.author_id,
      p.created_at,
      p.updated_at,
      coalesce(p.is_pinned, false) as is_pinned,
      coalesce(p.like_count, 0) as like_count,
      mp.display_name as author_display_name
    from public.posts p
    left join public.member_profiles mp on mp.id = p.author_id
    where coalesce(p.is_deleted, false) is false
      and (p_category = '전체' or p.category = p_category)
  )
  select * from base
  order by is_pinned desc, created_at desc, id desc
  limit greatest(p_limit, 1);
$$;

comment on function public.get_posts_preview(text, int)
  is 'Return lightweight post list with HTML-stripped preview to reduce payload.';

