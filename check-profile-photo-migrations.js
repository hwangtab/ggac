    ...
    try {
      ...
      if (bucketError) {
        ...
        if (profilesBucket) {
          ...
          console.log(`   - 파일 크기 제한: ${profilesBucket.file_size_limit ? Math.round(profilesBucket.file_size_limit / 1024 / 1024) + 'MB' : '제한없음'}`);
          console.log(`   - 허용 파일 타입: ${profilesBucket.allowed_mime_types ? profilesBucket.allowed_mime_types.join(', ') : '모든 타입'}`);
        } else {
    ...
    for (const functionName of functionsToCheck) {
      try {
        ...
        const { error: funcError } = await supabase.rpc(functionName, 
          functionName === 'get_profile_photo_stats' ? {} : { user_id: '00000000-0000-0000-0000-000000000000' }
        );
