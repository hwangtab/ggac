...
정리한 문서입니다. 각 단계에서 필요한 도구와 명령어, 참고 문서를 한 곳에
모았습니다.
...
3. 환경 변수 파일 생성
   ...
   ```
4. Supabase 및 외부 서비스 키를 `.env.local`에 추가
...
- 커밋 메시지:
  [Conventional Commits](https://www.conventionalcommits.org/ko/v1.0.0/) 형식을
  따릅니다.
...
`npx playwright install`로 브라우저를 설치해야 할 수 있습니다.
...
2. 미리보기 배포가 필요한 경우 `npm run vercel:preview` 사용.
3. 배포 후 `npm run deploy:notify`로 알림 전송 (선택).
...
- 필요시 추가 모니터링 도구(Sentry, Logtail 등)를 연동할 수 있습니다.
...
| 개발 서버가 실행되지 않음 | 환경 변수 설정 확인, 포트 충돌 여부 확인                      |
| 배포 실패                 | Vercel Dashboard의 빌드 로그 확인, 환경 변수 재확인           |
| Supabase 연결 오류        | 서비스 키 및 URL 확인, 네트워크 상태 점검                     |
