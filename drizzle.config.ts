 ...
 * **23개를 전부 지웠다**(실측: 23 → 0, 계획이 `SEARCH` → `SCAN`). 원인은
 * `0004`·`0005`가 만든 인덱스가 Drizzle 스키마에 `index()`로 선언돼 있지
 * 않아서, push가 그것들을 "스키마에 없는 잉여"로 보고 제거했기 때문이다.
 *
 * **2026-09-01에 그 23개를 전부 스키마에 선언했다**(`performanceIndexDeclarations.test.mjs`가
 * 지킨다). 그래서 인덱스가 통째로 날아가는 경로는 닫혔다. 그럼에도 이 가드를
 * 남기는 이유: push는 스키마에 없는 **다른** 것들(장래에 손으로 추가할 인덱스,
 * 트리거 등)에 대해 같은 일을 하고, 무엇보다 운영 스키마 변경을 마이그레이션
 * 파일로 남기지 않으면 다음 사람이 무엇이 적용됐는지 알 방법이 없다.
 *
