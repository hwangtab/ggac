...
- **Supabase 연동 및 조합원 가입 기능 구현 중**: Supabase `auth` 스키마와 별도로 조합원 정보를 관리하는 `public.member_profiles` 테이블을 사용하고 있습니다. 현재 신규 조합원 가입 시 `member_profiles` 테이블에 사용자 정보가 정상적으로 추가되도록 하는 로gic을 개발하고 있으며, RLS 정책 관련 이슈를 해결하고 있습니다.
