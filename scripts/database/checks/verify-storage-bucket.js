...
//
// Supabase Storage `profiles` 버킷의 존재를 확인하고 RPC로 만들기를 시도한다.
//
...
//
// 객체 저장소는 Supabase Storage가 아니라 Vercel Blob이다
// (`src/lib/storage/`). Supabase 버킷을 만들거나 확인할 이유가 없다.
//
...
async function verifyStorageBucket() {
  ...
  try {
    ...
    buckets.forEach(bucket => {
      console.log(
        `   - ${bucket.id} (공개: ${bucket.public}, 크기제한: ${bucket.file_size_limit ? Math.round(bucket.file_size_limit / 1024 / 1024) + 'MB' : '무제한'})`
      )
    })
    ...
    if (profilesBucket) {
      ...
      console.log(
        `   - 파일 크기 제한: ${profilesBucket.file_size_limit ? Math.round(profilesBucket.file_size_limit / 1024 / 1024) + 'MB' : '무제한'}`
      )
      console.log(
        `   - 허용 MIME 타입: ${profilesBucket.allowed_mime_types ? profilesBucket.allowed_mime_types.join(', ') : '모든 타입'}`
      )
    } else {
