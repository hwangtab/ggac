...
test('세 문서가 같은 기준선(130)을 말한다', () => {
  ...
  for (const [name, src] of [
    ...
    assert.match(src, /130 passed/, `${name}의 기준선이 사라지면 "몇 건이 정상인지"를 잃는다`)
    // 낡은 숫자가 **함께** 남아 있으면 위 match만으로는 못 잡는다.
    assert.doesNotMatch(
      src,
      /기준선[^\n]*(50|64|70|89|92|93|94|96|97|98|105|110|112|115|116|117|118|125) passed/,
