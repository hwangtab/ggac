...
const SCRIPT = 'scripts/verify-env.js'

// `@next/env`의 loadEnvConfig가 `.env*`를 읽어 들이므로, 테스트가 로컬 개발자의
// `.env.local`에 흔들리지 않도록 검사 대상 변수를 전부 명시적으로 지정한다.
// (`loadEnvConfig`는 이미 정의된 process.env 값을 덮어쓰지 않는다.)
const ALL_KEYS = [
...
function run(overrides) {
  ...
  try {
    const stdout = execFileSync('node', [SCRIPT], { encoding: 'utf8', env, stdio: 'pipe' })
