...
**실측 기준선(2026-09-25, 비조합원 개설자 경로를 연 뒤):** 총 147건 중
**146 passed, 0 failed, 1 skipped.** 정책 36(첨부 업로드)은 위 3단계의
`PUBLIC_BLOB_READ_WRITE_TOKEN`을 준 경우다 — 토큰 없이 돌리면 그 1건이
"토큰이 없으면 통과할 수 없다"로 실패하며, 이 저장소의 다른 변경과 무관하다
(토큰 없이 실측: 145 passed, 1 failed, 1 skipped). 늘어난 하나는 사무국이 대신 열어 준 캠페인을
...
**인가를 바꿨으면 `npm run test:e2e:authz`를 돌려라**(기준선 146 passed, 실행 절차는
