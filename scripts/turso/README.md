...
**실측 기준선(2026-09-26, 비조합원 개설자를 화면까지 들여보낸 뒤):** 총 148건 중
**147 passed, 0 failed, 1 skipped.** 정책 36(첨부 업로드)은 위 3단계의
`PUBLIC_BLOB_READ_WRITE_TOKEN`을 준 경우다 — 토큰 없이 돌리면 그 1건이
"토큰이 없으면 통과할 수 없다"로 실패하며, 이 저장소의 다른 변경과 무관하다.
늘어난 하나는 **화면** 쪽이다. 앞 회차가 연 것은 API뿐이어서, 미들웨어가
`/mypage` 전체를 승인·활성 조합원 전용으로 묶고 있다는 사실이 드러나지 않았다
— 비조합원 개설자는 메일에 실린 `/mypage/funding/{id}` 링크에서 그대로
`/register/pending`으로 튕겼다. 새 테스트는 브라우저로 그 주소에 들어가고,
개설 화면(`/mypage/funding/new`)과 나머지 마이페이지는 여전히 닫혀 있음을
함께 단언한다.

**직전 기준선(2026-09-25, 비조합원 개설자 API를 연 뒤):** 총 147건 중
**146 passed, 0 failed, 1 skipped.** (토큰 없이 실측: 145 passed, 1 failed,
1 skipped.) 늘어난 하나는 사무국이 대신 열어 준 캠페인을 **조합원이 아닌
개설자**가 직접 읽고 고치고 심사에 올리는 길이다(그러면서도 스스로 개설하지는
못하고, 남의 캠페인에도 닿지 못한다).
...
**실측 기준선(2026-09-25, 2차 적대 감사 수리 뒤):** 총 143건 중
**142 passed, 0 failed, 1 skipped.** 늘어난 것은 인증 관문 우회 둘, 개인정보
열람 기록 위조, 사무국 대리 환불·이행 되돌리기 라우트의 경계다.
...
내보내지 않는 것, 관리자 승인이 읽은 판을 요구하는 것.

**직전 기준선(2026-09-14, 정관 전용 화면을 낸 뒤):** 총 100건 중
**98 passed, 1 failed, 1 skipped.** 늘어난 1건은 정관 화면이 조합원·이사 양쪽에
열린다는 경계다.

**직전 기준선(2026-09-14, 조합원 화면을 다듬은 뒤):** 총 99건 중
**97 passed, 1 failed, 1 skipped.** 늘어난 1건은 회의 생성·수정 화면이
조합원에게 닫힌다는 경계다.

**직전 기준선(2026-09-14, 조합원에게 서류함을 열고 메뉴를 재배치한 뒤):** 총 98건 중
...
1건은 `authz-mailbox.spec.ts`의 첨부 다운로드 기록 테스트다 — 기록은
스트리밍 직전에만 남는데 이 스위트는 운영 Blob 오염을 피하려고 실제 Blob
...
**인가를 바꿨으면 `npm run test:e2e:authz`를 돌려라**(기준선 147 passed, 실행 절차는
...
**RPO는 24시간이 아니라 실무 기준 24~27시간으로 본다.** 백업은 매일 고정
시각(KST 03:00) 한 번만 돈다. "24시간"은 그 cron 간격일 뿐이고, 실제
유실 폭은 "장애 시점에서 *마지막으로 성공한* 백업까지 지난 시간"이다. 이
값이 24시간을 넘는 경우가 정상 범위에 있다 — GitHub Actions의 `schedule`
트리거는 정시 실행을 보장하지 않고 GitHub 쪽 부하가 크면 지연되거나
드물게 건너뛴다(GitHub 공식 문서에 명시된 동작). 즉 어제 백업이 03:00에
정확히 돌았어도 오늘 백업이 지연되면, 그사이 생긴 장애의 RPO는 24시간을
넘긴 채로 다음 백업을 기다리게 된다. 그래서 이 문서는 "RPO 24시간"이
아니라 "고정 1일 1회 스케줄이라 최악의 경우 24~27시간까지 늘어날 수
있다"를 기준으로 삼는다.

### 1b. **운영이 통째로 사라졌을 때** — 새 Turso 데이터베이스로 되찾기

위 실측(2026-08-27)은 전부 **로컬 `file:` 대상**이었다. "RTO가 근거 없다"는
지적(적대 감사, 2026-09-26)은 정확했다 — **새로 만든 원격 Turso 데이터베이스로
이 덤프를 되돌려 본 적이 그때까지 한 번도 없었다.** 실제로 돌려 보니 로컬
파일 대상과는 다른 층위의 문제가 셋 나왔다:

1. `client.executeMultiple(덤프_전체)`를 원격(Hrana/HTTP)에 그대로 보내면
   `SQLITE_TOOBIG`로 죽는다 — 한 요청의 SQL 본문 크기 상한을 넘는다.
2. 덤프 맨 앞 `PRAGMA foreign_keys=OFF;`가 **원격에서는 효과가 없다**(직후
   읽으면 여전히 1). 이 스키마는 참조 대상 테이블이 나중에 나오는
   `CREATE TABLE`이 있는데(임베디드 SQLite는 늘 허용하는 전제), 원격은 그
   자리에서 "no such table"로 죽는다 — **Turso 공식 CLI(`turso db shell <url>
   < dump.sql`)로 그대로 부어도 똑같이 죽는다.** 이 저장소만의 문제가
   아니라 이 덤프 형태 자체의 문제다.
3. 테이블 생성 순서를 고쳐도(`restore-from-dump.mjs`가 이제 FK 참조 대상이
   먼저 오도록 위상 정렬한다), **행 삽입 순서**가 남는다. `PRAGMA
   defer_foreign_keys=ON`으로 커밋 시점까지 미루면 되는데, 실측으로는 모든
   `CREATE TABLE`·`INSERT`가 끝까지 들어간 뒤 **COMMIT 한 줄에서만** FK
   위반으로 죽었고, 그 직후 `PRAGMA foreign_key_check`는 위반을 0건
   보고했다 — 데이터는 멀쩡한데 Turso 쪽 지연 제약 카운터가 원격 다중 요청
   복원 앞에서 어긋나는 것으로 보인다(Turso/libsql 쪽 문제로 보이며 이
   저장소 코드로 고칠 수 있는 범위 밖이다).

**그래서 새 원격 데이터베이스로 되찾는 정본 절차는 파일을 경유한다:**

```bash
# ① 위 1번 절차대로 최신 백업을 받아 로컬 file: 로 복원한다 — 이 경로는
#    검증됐다(29표/18,250행, 2026-08-27).
node scripts/turso/restore-from-dump.mjs "$D/latest.sql" "file:$D/restored.db"

# ② 그 SQLite 파일을 파일째로 새 Turso 데이터베이스로 만든다 — SQL을 다시
#    실행하는 게 아니라 파일을 직접 옮기므로 위 세 문제를 전부 비껴간다.
turso db create <새-데이터베이스-이름> --from-file "$D/restored.db"
```

`turso db create --from-dump`(로컬 SQL 파일을 바로 원격에 붓는 옵션)도
있지만 내부적으로 같은 SQL 재생 경로를 탈 가능성이 높아 검증하지 않았다 —
**`--from-file`(파일 자체를 옮기는 경로)을 쓴다.** `--from-file` 자체는 이
세션에서 실행하지 않았다(계정에 실제 데이터베이스를 만드는 동작이라 사전
승인 없이 하지 않았다) — 다음에 이 문서를 펴는 사람이 실행하고 결과를 여기
`실측(...)`로 남긴다.

**⚠ 별개로 발견한 것 — `inbound_emails` 한 행이 5.28MB다.** 메일함 원문(첨부
포함)이 Blob이 아니라 Turso 행에 그대로 들어간 것으로 보인다. 원격 엔진은
값 하나가 일정 크기를 넘으면(로컬 `turso dev` 기준 5MB대에서 재현)
`SQLITE_TOOBIG`로 거부한다 — 이런 행이 있으면 위 파일 이전 경로에서도
막힐 수 있다. 근본 해결은 수신 메일 본문·첨부를 Blob으로 옮기고 행에는
참조만 남기는 것이다(별도 작업).
...
**롤백 시 후속 작업**: `0001`·`0003`·`0004` 재적용(표 3개 + 인덱스 20개) →
`system_settings` 재구성.

> **⚠ `0002`는 재적용 대상이 아니다.** `0007`이 `board_meetings`에 `meeting_time`을
> 붙인 뒤로 `0002`의 표 재작성은 그 컬럼과 값, 그리고 `0004`가 만든
> `idx_board_agendas_meeting`·`idx_board_documents_category`를 함께 지운다.
> 예전에는 이 문단이 `0001`~`0004`라고 적혀 있었고, 그대로 따르면 이사회 회의
> 조회·생성·수정이 `no such column: meeting_time`으로 전부 500이 됐다
> (2026-09-01 실측·재현). 지금은 `0002` 안에 재실행 차단 단언이 있어 CHECK
> 위반으로 롤백되지만, **애초에 목록에 넣지 않는 것**이 맞다. `user_sessions`가 없어도 로그인은 죽지 않지만(`manageUserSession`이 `after()`
