    ...
    for (const filter of filters) {
      ...
      if (response.status === 401) {
        ...
        const statusAnalysis = {
          ...
          approved_active: members.filter(m => m.registration_status === 'approved' && m.is_active)
            .length,
          approved_inactive: members.filter(
...
WHERE is_suspended = true;
    ...
    console.log('\n🎯 분석 완료!')
    console.log(
      '다음 단계: Supabase 대시보드에서 위 SQL 쿼리들을 실행하여 정확한 데이터를 확인하세요.'
    )
  } catch (error) {
