...
const CooperativeInfo: React.FC<CooperativeInfoProps> = ({ data, errors, onChange }) => {
  ...
  return (
    <div className="bg-gray-50 rounded-lg p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* 월 조합비 */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">월 조합비 (원)</label>
          ...
          {errors.monthly_fee && <p className="mt-1 text-xs text-red-600">{errors.monthly_fee}</p>}
          {data.monthly_fee > 0 && (
            <p className="mt-1 text-xs text-gray-500">월 {formatCurrency(data.monthly_fee)}원</p>
          )}
        </div>

        {/* 조합원 상태 (읽기 전용 정보) */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">조합원 정보</label>
          ...
          <p className="mt-1 text-xs text-gray-500">조합원 정보는 관리자가 관리합니다.</p>
        </div>
      </div>
    </div>
  )
