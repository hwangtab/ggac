...
export async function GET(request: NextRequest) {
  return withRateLimit('ADMIN_API')(async () => {
    try {
      ...
      switch (trendType) {
        case 'activity':
          trendData = await getActivityTrends(supabase, period, weeks)
          break

        case 'users':
          trendData = await getUserTrends(supabase, period, weeks)
          break

        case 'engagement':
          trendData = await getEngagementTrends(supabase, period, weeks)
          break
...
async function getActivityTrends(supabase: any, period: string, weeks: number) {
  ...
  const series = weeklyStats?.map((week: any) => ({
  ...
  // 전체 트렌드 계산
  const totalTrend = calculateTrendDirection(series.map((s: any) => s.value))
  ...
  return {
    ...
    trends: {
      overall: totalTrend,
      byActionType: Object.keys(actionTypeGroups).reduce((acc: Record<string, any>, actionType: string) => {
        acc[actionType] = calculateTrendDirection(actionTypeGroups[actionType].map((item: any) => item.value))
        return acc
      }, {})
    },
    summary: {
      totalActivities: series.reduce((sum: number, item: any) => sum + item.value, 0),
      averageWeeklyActivities: series.reduce((sum: number, item: any) => sum + item.value, 0) / Math.max(weeks, 1),
      peakWeek: series.reduce((max: any, item: any) => item.value > (max?.value || 0) ? item : max, null)
    }
  }
}
...
async function getUserTrends(supabase: any, period: string, weeks: number) {
  ...
  return {
    ...
    summary: {
      ...
      peakActiveWeek: weeklyData.length > 0 ? weeklyData.reduce((max, week) => week.activeUsers > (max?.activeUsers || 0) ? week : max) : null
    }
  }
}
...
async function getEngagementTrends(supabase: any, period: string, weeks: number) {
  ...
  for (let i = 0; i < weeks; i++) {
    ...
    const weekActivities = engagementData?.filter((activity: any) => {
  ...
  return {
    ...
    summary: {
      totalEngagements: weeklyEngagement.reduce((sum, week) => sum + (week.totalEngagements || 0), 0),
      averageWeeklyEngagements: weeklyEngagement.reduce((sum, week) => sum + (week.totalEngagements || 0), 0) / weeks,
      mostEngagedWeek: weeklyEngagement.length > 0 ? weeklyEngagement.reduce((max, week) => (week.totalEngagements || 0) > (max?.totalEngagements || 0) ? week : max) : null
    }
  }
}
...
async function getPerformanceTrends(supabase: any, period: string, weeks: number) {
  ...
  for (let i = 0; i < weeks; i++) {
    ...
    const sessionDurations = weekSessions.map((session: any) => {
      ...
      const logoutTime = session.logout_at ? new Date(session.logout_at) : new Date(session.last_activity)
      return Math.max(0, Math.round((logoutTime.getTime() - loginTime.getTime()) / 60000)) // 분 단위
    })

    const avgSessionDuration = sessionDurations.reduce((sum: number, duration: number) => sum + duration, 0) / Math.max(sessionDurations.length, 1)
    ...
    weeklyPerformance.push({
      ...
      totalSessionTime: sessionDurations.reduce((sum: number, duration: number) => sum + duration, 0)
    })
  }
  ...
  return {
    ...
    summary: {
      ...
      overallAvgDuration: weeklyPerformance.reduce((sum, week) => sum + week.avgSessionDuration, 0) / weeks
    }
  }
}
 ...
 * 트렌드 방향 계산
 */
function calculateTrendDirection(values: number[]): { direction: 'up' | 'down' | 'stable', percentage: number, change: number } {
  if (values.length < 2) {
  ...
  if (Math.abs(percentage) > 5) { // 5% 이상 변화를 의미있는 트렌드로 간주
