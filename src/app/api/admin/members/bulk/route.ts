...
export async function POST(request: NextRequest) {
  try {
    ...
    // 작업 타입 검증
    const allowedOperations = ['bulk_approve', 'bulk_reject', 'bulk_activate', 'bulk_deactivate', 'bulk_suspend']
    ...
    try {
      ...
      // 보안 이벤트 로깅
      logSecurityEvent('BULK_OPERATION_COMPLETED', {
      ...
      await supabase
        ...
        .update({
          status: 'failed',
          completed_at: new Date().toISOString(),
          error_message: operationError instanceof Error ? operationError.message : '알 수 없는 오류',
          results: {
            success_count: successCount,
            error_count: errorCount,
            details: results
    ...
    logSecurityEvent('BULK_OPERATION_ERROR', { 
