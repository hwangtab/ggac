  ...
  try {
    ...
    for (const setting of settingsToReset) {
      try {
        ...
        if (updateError) {
          console.error(`Setting reset error for ${setting.category}.${setting.setting_key}:`, updateError)
    ...
    // 보안 이벤트 로깅
    logSecurityEvent('ADMIN_SETTINGS_RESET_TO_DEFAULTS', {
      adminId: session.user.id,
      resetType,
      category,
      reset: resetResults,
      errors: errorResults
    }, 'high') // 기본값 복원은 높은 보안 등급
    ...
    logSecurityEvent('ADMIN_SETTINGS_RESET_ERROR', { 
