...
const FormField: React.FC<FormFieldProps> = ({
  ...
  const getInputClasses = () => {
    const baseClasses = "w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 transition-all duration-200 disabled:bg-gray-50 disabled:text-gray-500";
      ...
      <div className="relative">
        ...
        {state !== 'default' && (
          <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
            {state === 'error' ? (
              <svg className={`h-5 w-5 ${getIconClasses()}`} fill="currentColor" viewBox="0 0 20 20">
