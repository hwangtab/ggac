...
const LazyParticles = memo(
  ({
    ...
    const [ParticleComponent, setParticleComponent] = useState<ComponentType<ParticleProps> | null>(
      null
    )
    ...
    return (
      <div ref={targetRef} className="absolute inset-0 pointer-events-none" style={{ zIndex: 30 }}>
        {/* 로딩 상태 */}
            ...
            fallback={({ error, reset, retryCount, componentName }) => (
              <div className="absolute inset-0 flex items-center justify-center bg-black/5 backdrop-blur-sm rounded-lg">
                <div className="text-center text-white/50 p-4">
                  ...
                  {retryCount >= 2 && <p className="text-xs opacity-75">자동 복구 실패</p>}
                </div>
              </div>
            )}
          ...
          >
            {renderPerf.avgRenderTime > 16 && (
              <div className="text-yellow-400">
                Slow Render: {renderPerf.avgRenderTime.toFixed(1)}ms
              </div>
            )}
