...
export const useAdvancedFilter = ({
  ...
  const emitChange = useCallback(
    (newGroup: FilterGroup, newSorts: SortCondition[]) => {
      onChange({
        filters: newGroup,
        sorts: newSorts,
      })
  ...
  const hasActiveFilters = useCallback((group: FilterGroup): boolean => {
    ...
    const hasNestedFilters =
      group.groups && group.groups.length > 0 && group.groups.some(g => hasActiveFilters(g))
  ...
  const addCondition = useCallback(
    (group: FilterGroup, groupPath: number[] = []) => {
  ...
  const addGroup = useCallback(
    (parentGroup: FilterGroup, groupPath: number[] = []) => {
  ...
  const updateCondition = useCallback(
    (conditionIndex: number, updates: Partial<FilterCondition>, groupPath: number[] = []) => {
  ...
  const removeCondition = useCallback(
    (conditionIndex: number, groupPath: number[] = []) => {
      const updatedGroup = { ...filterGroup }
      let targetGroup = updatedGroup
  ...
  const removeGroup = useCallback(
    (groupIndex: number, groupPath: number[] = []) => {
      const updatedGroup = { ...filterGroup }
      let targetGroup = updatedGroup
  ...
  const updateGroupOperator = useCallback(
    (operator: 'AND' | 'OR', groupPath: number[] = []) => {
      const updatedGroup = { ...filterGroup }
      let targetGroup = updatedGroup
  ...
  const updateSort = useCallback(
    (index: number, updates: Partial<SortCondition>) => {
      const newSorts = sorts.map((sort, i) => (i === index ? { ...sort, ...updates } : sort))
      setSorts(newSorts)
      emitChange(filterGroup, newSorts)
    },
  ...
  const loadPreset = useCallback(
    (preset: { filters: FilterGroup; sorts: SortCondition[] }) => {
