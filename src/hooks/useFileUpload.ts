...
export const useFileUpload = ({
  ...
  // 검증 설정 결정
  const validationConfig = typeof validationProfile === 'string' 
  ...
  const handleFileSelect = useCallback((files: FileList | File[]) => {
    ...
    if (validation.rejectedFiles.length > 0) {
      const rejectedMessages = validation.rejectedFiles.map(({ file, errors }) => 
  ...
  // 첨부파일 업로드
  const uploadAttachments = useCallback(async (postId: string): Promise<void> => {
