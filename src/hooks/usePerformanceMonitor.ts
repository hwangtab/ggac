...
export const usePerformanceMonitor = (options: PerformanceMonitorOptions = {}) => {
  ...
  const measureFrame = useCallback(
    (currentTime: number) => {
      // 활성화 상태를 런타임에 체크
      if (!enabled || (devOnly && process.env.NODE_ENV !== 'development') || !isActiveRef.current)
        return
  ...
  const generateReport = useCallback(() => {
    ...
    if (report.summary.errorCount > 5) {
      report.recommendations.push(
        `${report.summary.errorCount}개의 에러가 발생했습니다. 안정성 개선이 필요합니다.`
      )
    }

    if (report.summary.criticalErrors > 0) {
      report.recommendations.push(
        `${report.summary.criticalErrors}개의 치명적 에러가 감지되었습니다. 즉시 수정이 필요합니다.`
      )
    }
