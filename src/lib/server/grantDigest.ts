...
/**
 * 한 회차의 공고 풀 상한. {@link CAP}은 **메일 한 통**의 상한이고 이것은 **풀 전체**의
...
/**
 * 제목 기반 중복 제거({@link normalizedTitleKey})에 볼 과거 회차 수. **`key` 축보다
 * 짧다.**
 *
 * `key` 축은 같은 공고가 두 번 나가는 것을 막는 정확한 규칙이라 캘린더가 보는 범위
 * (26주) 전체에 걸어도 안전하다. 제목 축은 근사이고 서로 다른 사업을 같은 것으로 볼 수
 * 있다(연례 공모의 제목은 해마다 거의 같다) — 창을 길게 잡을수록 그 오차단이 오래
 * 지속된다. 재공고가 원공고 뒤에 붙는 간격은 실측(양평문화자원 W36→W37)에서 한 주였다.
 */
export const TITLE_DEDUPE_WEEKS = 12
 ...
 *   문자열이 없기 때문이다.
 * - `예술감독`: 2026-W37 실측 3번 「2026년 꿈의 극단 안산 예술감독 모집」을 잡는다.
 *   채용 공고인데 `category='grant'`·`biz_type='인력, 기타'`라 다른 규칙에 안 걸려
 *   게시글·메일·캘린더에 전부 실렸다. 한때 '모집'을 넣어 이 부류를 통째로 잡으려 했으나
 *   실데이터 집계에서 오차단이 6:1이었다(「참여단체 모집」·「참여자 모집」처럼 유효한
 *   공모가 훨씬 많다) — 표적을 좁혀 직함으로 잡는다.
 ...
 *
 * **앞으로의 초안에만 효과가 있다.** 이미 발행된 회차에 나란히 실린 중복(실측 W36·W37의
 * 양평문화자원 두 건)은 그대로 남는다 — 캘린더는 발행된 회차를 그대로 읽으므로 그 칸에는
 * 지금도 두 건이 찍혀 있다. 지난 회차를 고쳐 쓰지 않는다.
 *
 ...
 * 이메일 제목과 본문.
 *
 * 잘린 나머지를 **캘린더**로 안내한다 — 게시글이 아니다. 게시글은 조합 기본 관심사로
 * 좁혀 렌더되므로(`grantPublish.ts`), 개인 관심사로 고른 이 메일의 나머지가 게시글에
 * 없을 수 있다. 캘린더(`/api/mypage/calendar`)는 이 메일과 **같은**
 * `effectiveInterests`·`matchesInterests`로 거르므로 그 안내는 항상 참이다.
 *
...
export function renderDigestEmail(
  ...
  const truncatedNote =
    truncatedFrom > active.length
      ? `<p style="font-size: 13px; color: #b45309; margin: 0 0 16px">마감이 가까운 ${active.length}건만 담았습니다. 나머지 ${truncatedFrom - active.length}건은 마이페이지 캘린더에서 볼 수 있습니다.</p>`
      : ''
