...
export type ReconcileResult =
   ...
   * 사람을 부른다.
   *
   * **사람이 할 일은 사무국 대리 환불 화면에서 그 후원을 환불하는 것이다.**
   * 그 경로는 취소 금액을 싣지 않고 토스를 부르므로(`cancelPayment`에
   * `cancelAmount`를 넘기지 않는다 = 남은 금액 전액 취소), 이미 일부가 취소된
   * 결제에서는 **남은 금액만** 마저 나간다. 원장은 그제야 전액 환불로 맞춰지고
   * (원장의 기존 누적 취소액은 0이라 조건도 통과한다) 대사가 다음번에는 이
   * 후원을 `refunded`로 만나 그냥 지나간다.
   */
