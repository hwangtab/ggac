export type SecurityEventType =
  ...
  // 같은 일이 예매에서 일어난 경우 — 하루가 지나도 풀리지 않은 좌석 선점.
  // 승인된 결제가 붙어 있는데 좌석이 없는 건이 섞일 수 있고, 티켓은 자리가
  // 한정돼 있어 그 행이 남아 있는 동안 재고 보고도 어긋난다.
  | 'TICKET_STUCK_PENDING_HOLDS'
  // 같은 일이 조합비에서 일어난 경우 — 대사가 하루가 지나도 판정하지 못한
  // 대기 결제. 승인된 회비가 미납으로 남아 다음 달 자동청구가 그 달을 다시
  // 걷을 수 있다.
  | 'DUES_STUCK_PENDING_PAYMENTS'
  // 준비와 확정 사이에 그 달이 이미 납부로 바뀌어, 승인을 보내지 않고 막은
  // 경우. 카드는 긁히지 않았다 — 막지 않았다면 같은 달을 두 번 걷고 남는 일은
  // 환불뿐이었다. 왜 그 달이 이미 납부였는지(사무국 수기 기록·창 두 개)는
  // 사람이 봐야 알 수 있으므로 남긴다.
  | 'DUES_CONFIRM_DUPLICATE_MONTH'
  // 결제는 확정됐는데 그 달이 납부로 바뀌지 않은 경우. 돈은 들어왔는데 회비는
  // 미납으로 남아 다음 달 청구가 같은 달을 또 걷는다 — 조용히 지나가면 그때까지
  // 아무도 모른다.
