...
class ApiPerformanceMonitor {
  ...
  recordMetric(
    ...
    // 메모리 사용량 (개발 환경에서만)
    const memoryUsage = process.env.NODE_ENV === 'development' 
      ? {
  ...
  private checkPerformanceAlerts(metric: ApiMetrics): void {
    ...
    // 에러 응답 경고
    if (!metric.success) {
      console.warn(`❌ API error: ${metric.endpoint} (${metric.statusCode}) - ${metric.error || 'Unknown error'}`);
    }
...
export const getApiHealth = () => {
  ...
  return {
    status: overview.successRate > 95 ? 'healthy' : 
            overview.successRate > 80 ? 'degraded' : 'unhealthy',
