...
class BuildPerformanceTracker {
  ...
  private printSummary() {
    ...
    if (dependencies) {
      console.log(
        `📚 Dependencies: ${dependencies.total} total (${dependencies.production} prod, ${dependencies.development} dev)`
      )
    }
  ...
  private checkPerformanceWarnings() {
    ...
    if (memoryUsage && memoryUsage.heapUsed > 1000) {
      // 1GB 이상
      warnings.push(`⚠️  High memory usage (${memoryUsage.heapUsed}MB)`)
    }

    if (bundleStats && bundleStats.totalSize > 5000) {
      // 5MB 이상
      warnings.push(`⚠️  Large bundle size (${bundleStats.totalSize}KB)`)
    }

    if (bundleStats && bundleStats.largestChunks.some(chunk => chunk.size > 1000)) {
      // 1MB 이상 청크
  ...
  async analyzePerformanceTrends(): Promise<{
    ...
    const olderBundleSize =
      older.reduce((sum, m) => sum + m.bundleStats.totalSize, 0) / older.length
    const newerBundleSize =
      newer.reduce((sum, m) => sum + m.bundleStats.totalSize, 0) / newer.length

    const olderMemoryUsage =
      older.reduce((sum, m) => sum + m.memoryUsage.heapUsed, 0) / older.length
    const newerMemoryUsage =
      newer.reduce((sum, m) => sum + m.memoryUsage.heapUsed, 0) / newer.length
