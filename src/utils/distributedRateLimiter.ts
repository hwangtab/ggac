...
export const DISTRIBUTED_RATE_LIMIT_CONFIGS = {
  ...
  // 자체 가입 (시간당 10회)
  //
  // 계정을 **만드는** 자리는 읽기와 같은 잣대로 잴 수 없다. 이 라우트가
  // AUTH_API(분당 10회 = 시간당 600회)를 쓰는 동안, 인증 없이 한 IP에서
  // 하루 1만 개가 넘는 계정을 만들 수 있었다. 생기는 것은 스팸 계정만이
  // 아니다 — 남의 주소로 먼저 가입해 그 사람의 가입을 막는 이메일 선점,
  // 그리고 사무국이 손으로 훑어야 하는 승인 대기 목록의 오염이 같은 숫자
  // 하나로 열려 있었다.
  //
  // 사람이 가입하는 횟수는 시간당 한 번을 넘지 않는다. 오타·검증 실패로
  // 다시 보내는 몫까지 감안해도 10회면 넉넉하고, 같은 공유망(사무실·행사장)에서
  // 여럿이 가입하는 경우까지 덮는다.
  SIGNUP: {
    name: 'signup',
