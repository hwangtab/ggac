 ...
 *
 * NOTE: 한글 음역은 자체 매핑 + 한글 자모 분해 알고리즘으로 구현되어 있다.
 * 정확도가 중요해진다면 `hangul-romanization` 또는 `korean-romanization` 같은
 * 검증된 라이브러리로 교체하는 것을 검토할 수 있다. 현 시점에는
 * 의존성 부담을 피하기 위해 자체 구현을 유지한다.
 */
