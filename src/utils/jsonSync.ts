...
export async function updateArtistInJsonFile(artistSlugOrId: string, updateData: any) {
  try {
    ...
    // 해당 아티스트 찾기 (slug 우선, 그 다음 ID로 찾기)
    const artistIndex = artists.findIndex(
      artist => artist.slug === artistSlugOrId || artist.id === artistSlugOrId
    )

    if (artistIndex === -1) {
      console.warn(`Artist with slug/ID ${artistSlugOrId} not found in JSON file`)
      return false
    }
    ...
    console.log(`Successfully updated artist ${artistSlugOrId} in JSON file`)
    return true
  } catch (error) {
