-- Fix: Postgres cannot change return type with CREATE OR REPLACE when using OUT params
-- Drop and recreate the function with the new return columns (attachment stats)

drop function if exists public.get_posts_preview(text, int);

create function public.get_posts_preview(p_category text default '전체', p_limit int default 20)
returns table (
  id uuid,
  title text,
